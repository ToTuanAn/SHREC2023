...
def main():
    train_set = TextPointCloudDataset(root_dir='/home/totuanan/Workplace/SHREC2023/SHREC2023/dataset',
                                      pc_transform=train_transforms, type='train')
    train_loader = DataLoader(dataset=train_set, batch_size=4, shuffle=True)

    validation_set = TextPointCloudDataset(root_dir='/home/totuanan/Workplace/SHREC2023/SHREC2023/dataset',
                                           pc_transform=validation_transforms, type='validation')
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        for i, data in enumerate(validation_loader):
            ...
            true_point_cloud_embedding, \
                ...
                text_queries_embedding = model.forward({'true_point_cloud': true_point_cloud,
                                                        'false_point_cloud': false_point_cloud,
                                                        'text_queries': {
                                                             'input_ids': input_ids,
                                                             'attention_mask': attention_mask
            ...
            positive_eucledian_distance = F.pairwise_distance(true_point_cloud_embedding, text_queries_embedding)
            negative_eucledian_distance = F.pairwise_distance(false_point_cloud_embedding, text_queries_embedding)
